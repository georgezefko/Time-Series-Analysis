...
class TimeSeriesFeatureEngineering:
    ...
    def roll_dataset(self, frame, y_column):
        frame['tsfresh_id'] = frame.reset_index().iloc[:,0] #1
    ...
    def class_imbalance_fix(self, X, Y, sampling):
        smote = SMOTE( \
            random_state = self.random_state, \
            k_neighbors = self.smote_neighbours, \
            sampling_strategy = sampling)#,self.smote_ratio if ratio is None else ratio)
